#Модуль используется при переключении между ролями: 
#меняет department_id соответствующий подразделению текущей роли
module CurrentDepartmentRedirect
  ...
  def redirect_with_proper_department(fallback: root_path)
    ...
    begin
      ...
      # Проверяем, нужен ли менять department_id для этого маршрута
  ...
  def route_requires_department?(route_params)
    #Список всех контроллеров, в которых следует подменять department_id
    department_dependent_routes = [
      ...
      {controller: 'department_resources/users', action: 'edit_roles'},
