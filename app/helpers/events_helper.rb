...
def render_event_actions(event)
  ...
  # Управление участием
  event_department = event_department_for(event, current_department)

  if event_department  
    case event_department.status
    when 'offered'
      # Кнопка утверждения для предложенного участия
      ...
      if can?(:reject, event_department)
        buttons << link_to(reject_offered_event_department_path(event_department), 
                      ...
                      data: { confirm: "Отклонить участие в мероприятии '#{event.title}'?" }) do
          content_tag(:i, '', class: 'times icon')
        end
      end
      
    when 'approved'
      # Кнопка редактирования для утвержденного участия
      if can?(:update, event_department)
        buttons << button_tag(
          content_tag(:i, '', class: 'edit icon'),
      ...
      if can?(:reject, event_department)
        buttons << link_to(reject_approved_event_department_path(event_department), 
                      ...
                      data: { confirm: "Отменить участие в мероприятии '#{event.title}'? Это действие нельзя отменить." }) do
          content_tag(:i, '', class: 'times icon')
        end
      end
    end
    ...
    # Нет участия - кнопки предложения/назначения
    temp_ed = OfferedEventDepartment.new(event: event, department: current_department)
    
if can?(:offer, temp_ed)
  buttons << button_tag(
    ...
    if can?(:assign, temp_ed)
      buttons << link_to(assign_event_path(event), 
                    method: :post,
                    class: 'ui icon teal button',
                    title: 'Назначить участие',
                    data: { confirm: "Назначить участие подразделению '#{current_department.name}' в мероприятии '#{event.title}'?" }) do
